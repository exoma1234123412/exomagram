...
`${newInvitations.length} invitaci${newInvitations.length === 1 ?"on":"ones"} enviada${newInvitations.length === 1 ?"":"s"} correctamente.`);
 ...
 <div className="w-9 h-9 bg-primary flex items-center justify-center shadow-blue-600/25">
 ...
 <DialogDescription>
 {orgName
 ?`Invita miembros a ${orgName}. Los invitados recibirán acceso al equipo.`:"Invita nuevos miembros a tu organización."}
 </DialogDescription>
 </DialogHeader>
 ...
 <Input
 id="invite-emails"placeholder="email1@ejemplo.com, email2@ejemplo.com"value={emailInput}
 onChange={(e) => {
 ...
 <div className="bg-accent/30 p-3 space-y-1.5">
 ...
 {success && (
 <div className="bg-green-50 dark:bg-green-950/20 border border-green-200/60 dark:border-green-800/40 p-3 flex items-center gap-2.5">
 ...
 className="w-full h-10 bg-primary hover:from-blue-700 hover:to-blue-800 text-white border-0 hover:shadow-blue-600/40 transition-all duration-300 font-semibold">
 {sending ? (
"Enviando...") : (
 <>
 ...
 <div
 key={inv.token}
 className="flex items-center justify-between bg-accent/30 px-3 py-2.5 group">
 <div className="flex items-center gap-2.5 min-w-0">
 <div className="w-8 h-8 bg-gradient-to-br from-blue-100 to-sky-100 dark:from-blue-900/40 dark:to-sky-900/30 flex items-center justify-center shrink-0">
 ...
 <Button
 variant="ghost"size="icon-xs"onClick={() => handleCopyLink(inv.token)}
 className="opacity-0 group-hover:opacity-100 transition-opacity"title="Copiar enlace de invitación">
 {isCopied ? (
 ...
 <Button
 variant="ghost"size="icon-xs"onClick={() => handleCancelInvitation(inv.token)}
 className="opacity-0 group-hover:opacity-100 transition-opacity text-destructive hover:text-destructive"title="Cancelar invitación">
 <X className="w-3 h-3"/>
 </Button>
 </div>
 </div>
...
"text-[10px] rounded-md",
 inv.status ==="accepted"?"bg-green-600":"text-muted-foreground border-muted")}
 >
