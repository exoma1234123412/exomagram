...
export function EmptyChair({ orgId }: { orgId: string }) {
 ...
 key={m.profile.id}
 className={cn(
"flex items-center gap-2.5 px-3 py-2.5 border transition-all",
...
"text-sm font-semibold",
 ...
 {m.hoursLogged === 0
 ?"0 horas hoy": m.lastActiveHour
 ?`Última actividad: ${m.lastActiveHour > 12 ? m.lastActiveHour - 12 : m.lastActiveHour}${m.lastActiveHour >= 12 ?"pm":"am"} (${m.gapHours}h sin actividad)`:`${m.hoursLogged}h registradas`}
 </p>
 </div>
 </div>
 );
