...
export function WorkSessionTracker({ orgId }: { orgId: string }) {
  ...
  return (
    <Card className={cn(
      "transition-all",
      isActive && !idleWarning && "border-green-300 dark:border-green-800 shadow-lg shadow-green-500/10",
      idleWarning && "border-red-300 dark:border-red-800 shadow-lg shadow-red-500/10 animate-pulse",
    )}>
      <CardContent className="p-5">
        ...
        {/* Idle warning */}
        {idleWarning && (
          <div className="bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800 rounded-xl p-3 mb-3 flex items-start gap-2">
                ...
                {(Object.keys(CATEGORIES) as WorkCategory[]).filter((k) => k !== "break").map((key) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "px-2.5 py-1.5 rounded-lg text-xs transition-all flex items-center gap-1",
                        category === key
                          ? "bg-primary/10 ring-2 ring-primary font-semibold"
                          : "bg-accent/40 hover:bg-accent opacity-70 hover:opacity-100"
                      )}
                    >
                      {cat.emoji} {cat.label}
                    </button>
