...
const WEATHER_CONFIG: Record<Weather, { emoji: string; label: string; color: string; bg: string }> = {
  sunny: { emoji: "☀️", label: "Excelente", color: "text-yellow-600", bg: "bg-yellow-50 dark:bg-yellow-950/15 border-yellow-200/60 dark:border-yellow-800/40" },
  ...
  cloudy: { emoji: "☁️", label: "Regular", color: "text-gray-500", bg: "bg-gray-50 dark:bg-gray-950/15 border-gray-200/60 dark:border-gray-800/40" },
  stormy: { emoji: "⛈️", label: "Bajo", color: "text-red-600", bg: "bg-red-50 dark:bg-red-950/15 border-red-200/60 dark:border-red-800/40" },
};
...
export function MoodWeather({ orgId }: { orgId: string }) {
  ...
  return (
    <div className={cn(
      "flex items-center gap-3 p-3 rounded-2xl text-sm mb-4 border transition-all",
      config.bg
    )}>
