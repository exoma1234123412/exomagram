...
export function QuickLog({ orgId }: { orgId: string }) {
 ...
 <div className="w-8 h-8 bg-yellow-100 dark:bg-yellow-900/30 flex items-center justify-center">
 ...
 className={cn(
"w-8 h-8 text-sm transition-all flex items-center justify-center",
 category === key
 ?"bg-primary/10 dark:bg-primary/20 ring-2 ring-primary scale-110":"bg-accent/40 hover:bg-accent opacity-60 hover:opacity-100")}
 title={cat.label}
 >
 {cat.emoji}
 </button>
 ...
 className="flex-1"disabled={!category}
 minLength={10}
 />

 <Button type="submit"size="icon"disabled={loading || !category || title.length < 10} className="bg-primary hover:from-blue-700 hover:to-blue-800 text-white border-0">
