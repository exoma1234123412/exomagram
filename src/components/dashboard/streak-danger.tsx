...
export function StreakDanger({ orgId }: { orgId: string }) {
 ...
 const tierConfig = {
 legendary: { label:"NIVEL IV", bg:"", border:"border-amber-500/40", text:"text-amber-500"},
 epic: { label:"NIVEL III", bg:"", border:"border-purple-500/30", text:"text-purple-400"},
 rare: { label:"NIVEL II", bg:"", border:"border-orange-500/30", text:"text-orange-500"},
 common: { label:"NIVEL I", bg:"", border:"border-primary/20", text:"text-primary"},
 };
...
"border p-4 mb-8 transition-all duration-300",
 isDanger
 ?"bg-red-500/5 border-red-500/40": tier.border,
...
"w-5 h-5 text-orange-500",
 ...
 <span className="text-xs font-mono font-medium text-muted-foreground uppercase">días</span>
 <span className={cn(
"text-[8px] font-mono font-bold px-2 py-0.5 uppercase tracking-[0.15em] border",
 isDanger
 ?"border-red-500/40 text-red-500 bg-red-500/10":`${tier.border} ${tier.text}`)}>
...
"p-3 mt-1 border",
 isCritical
 ?"bg-red-500/8 border-red-500/30":"bg-yellow-500/5 border-yellow-500/20")}>
...
"text-[11px] font-mono",
 ...
 ?`Si pierdes tu racha de ${data.currentStreak} días, te tomará MÁS DE UN MES reconstruirla.`: data.currentStreak >= 14
 ?`${data.currentStreak} días de trabajo tirados a la basura. ¿De verdad vas a dejar que pase?`: data.currentStreak >= 7
 ?`Llevas ${data.currentStreak} días. No lo arruines ahora.`:`Registra al menos una hora para mantener tu racha.`}
 </p>
