...
export function ActivityTicker({ orgId }: { orgId: string }) {
 ...
 return (
 <div className="relative overflow-hidden mb-4 bg-accent/40 border">
 <div
 ref={tickerRef}
 className="flex items-center gap-6 py-2 px-4 animate-ticker whitespace-nowrap"style={{
 ...
 <span className="text-muted-foreground">{timeAgo(entry.logged_at ?? entry.created_at)}</span>
 <span className="text-muted-foreground/20 mx-2">•</span>
 </span>
 );
 })}
 </div>
 <style jsx>{`@keyframes ticker {
