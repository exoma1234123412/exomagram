      ...
      <div className="max-w-3xl mx-auto px-4 py-2.5">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className={cn(
            ...
            data.imLast
              ? "bg-red-500/15 dark:bg-red-950/30"
              : data.myRank <= 3
              ? "bg-green-500/10 dark:bg-green-950/30"
              : "bg-muted/30"
          )}>
          ...
          {/* Surveillance eye */}
          <Eye className="w-3.5 h-3.5 text-muted-foreground/40 animate-eye-blink shrink-0 hidden sm:block" />
        </div>
          ...
          <div className="flex items-center gap-1 mt-1.5 overflow-hidden">
            {data.teamHours.slice(0, 5).map((member, i) => (
              <div key={i} className="flex items-center gap-1 text-[9px] text-muted-foreground">
                ...
                {i < Math.min(4, data.teamHours.length - 1) && <span className="text-muted-foreground/30">·</span>}
              </div>
            ))}
          </div>
        )}
