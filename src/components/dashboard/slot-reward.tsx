...
const REWARDS = [
  ...
  { match: "🛡️🛡️🛡️", label: "¡ESCUDO TOTAL!", description: "Inmunidad a penalización hoy", rarity: "epic" },
...
export function SlotReward({ show, onClose }: SlotRewardProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className={cn(
        "relative bg-card rounded-2xl border shadow-2xl p-6 max-w-sm w-full mx-4",
          ...
          {reels.map((symbol, i) => (
            <div
              ...
              className={cn(
                ...
                settled[i]
                  ? "border-primary/30 bg-primary/5 animate-slot-settle"
        ...
        {phase === "result" && (
          <button
            onClick={handleClose}
            className={cn(
              "w-full mt-4 py-2.5 rounded-xl font-semibold text-sm transition-all",
              isJackpot
