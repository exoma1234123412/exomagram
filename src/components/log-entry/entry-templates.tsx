...
export function EntryTemplates({ onApply }: EntryTemplatesProps) {
 ...
 key={t.id}
 onClick={() => onApply(t)}
 className="inline-flex items-center gap-1 px-2 py-1 text-[10px] font-medium bg-accent/40 hover:bg-accent transition-colors"title={`${cat.label}: ${t.title}`}
 >
 <span>{cat.emoji}</span>
 {t.name}
 </button>
...
export function ManageTemplatesDialog({ open, onOpenChange }: ManageTemplatesDialogProps) {
 ...
 const cat = CATEGORIES[t.category];
 return (
 <div key={t.id} className="flex items-center gap-2 p-2 bg-accent/40 border transition-all duration-300">
...
"px-2 py-1 rounded text-[10px] font-medium",
 category === key ?"bg-primary text-white":"bg-muted hover:bg-muted/80")}
 >
 ...
 <Button onClick={handleAdd} size="sm"disabled={!name.trim() || !title.trim()} className="w-full gap-2 bg-primary hover:from-blue-700 hover:to-blue-800 text-white border-0">
