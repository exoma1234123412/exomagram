...
"font-mono text-[10px] font-bold px-1.5 py-0.5 border",
 ...
 <DataRow
 label="Fecha"icon={<Calendar className="w-3 h-3"/>}
 value={format(parseISO(entry.date),"EEEE d 'de' MMMM yyyy", { locale: es })}
 />
 <DataRow
 label="Hora"icon={<Clock className="w-3 h-3"/>}
 ...
 {entry.project && (
 <DataRow
 label="Proyecto"icon={<FolderKanban className="w-3 h-3"/>}
 value={
 ...
 <DataRow
 label="Estado"icon={hasProof ? <Shield className="w-3 h-3"/> : <ShieldX className="w-3 h-3"/>}
 value={
 <span
 className={cn(
"font-mono text-[11px] font-bold",
 ...
 >
 {hasProof
 ?`${entry.proof_urls!.length} enlace${entry.proof_urls!.length > 1 ?"s":""}`:"SIN EVIDENCIA"}
 </span>
 }
 />
 ...
 <a
 href={url}
 target="_blank"rel="noopener noreferrer"className="font-mono text-[11px] text-primary hover:underline truncate block">
 {hostname} &rarr;
 </a>
 }
 />
 ...
 <DataRow
 label="Animo"icon={<Brain className="w-3 h-3"/>}
 value={
 entry.mood !== null ? (
 ...
 <span className="font-mono text-[11px] text-muted-foreground">NO REPORTADO</span>
 )
 }
 mono
 />
 <DataRow
 label="Energia"icon={<Zap className="w-3 h-3"/>}
 value={
 entry.energy !== null ? (
 <span className="font-mono text-[11px]">
 ...
 <DataRow
 label="Tardia"icon={<AlertTriangle className="w-3 h-3"/>}
 value={
 entry.is_late ? (
 ...
 />
 <DataRow
 label="Logged at"icon={<Timer className="w-3 h-3"/>}
...
"font-mono text-[11px] tabular-nums",
 responseMinutes > 60
 ?"text-red-500": responseMinutes > 30
 ...
 <DataRow
 label="Titulo chars"icon={<Hash className="w-3 h-3"/>}
 value={entry.title.trim().length}
 mono
 />
 <DataRow
 label="Desc palabras"icon={<MessageSquare className="w-3 h-3"/>}
 value={descWords}
 mono
 />
 <DataRow
 label="Desc chars"icon={<Hash className="w-3 h-3"/>}
 value={descChars}
 mono
 />
 {report.details.descriptionSimilarity > 0 && (
 <DataRow
 label="Similitud"value={
 <span
 className={cn(
"font-mono text-[11px] tabular-nums",
 ...
 <DataRow
 label="Estado"icon={<CheckCircle2 className="w-3 h-3"/>}
 value={
 <span
 ...
 <DataRow
 label="Nota"icon={<PenLine className="w-3 h-3"/>}
 value={
 ...
 <DataRow
 label="Created"icon={<Calendar className="w-3 h-3"/>}
 value={format(parseISO(entry.created_at),"dd/MM/yyyy HH:mm:ss")}
 mono
 />
 <DataRow
 label="Updated"icon={<RefreshCw className="w-3 h-3"/>}
 value={
