...
//
// Inspired by:
// - Military briefing orders: "State your mission before you execute."
// - Amazon's "working backwards" memo: Write what you'll accomplish
...
// 1) Pre-commitment bias: Once you declare "8 hours of Deep Work,"
//    cognitive dissonance makes it painful to do less.
// 2) Specificity effect: "¿Qué vas a lograr?" forces concrete goals
//    vs vague "I'll work on stuff."
// 3) Obstacle anticipation: "¿Qué podría impedirlo?" activates
...
export function MorningIntentionGate() {
  ...
  const categoryEntries = useMemo(() => {
    return Object.entries(CATEGORIES).filter(
      ([key]) => key !== "break" && key !== "blocked"
    ) as [WorkCategory, (typeof CATEGORIES)[WorkCategory]][];
  }, []);
        ...
        >
          <DialogHeader>
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-amber-100 to-orange-100 dark:from-amber-900/30 dark:to-orange-900/30 flex items-center justify-center shadow-lg shadow-amber-500/10">
          ...
          {/* Yesterday's summary */}
          {yesterdaySummary && (
            <div className="bg-accent/40 rounded-xl p-3.5 flex items-center gap-3 text-sm">
                ...
                <span
                  className={cn(
                    ...
                    yesterdaySummary.proofPercentage >= 80
                      ...
                      : yesterdaySummary.proofPercentage >= 50
          ...
          {/* No-dismiss warning */}
          <div className="bg-amber-50/80 dark:bg-amber-950/15 border border-amber-200/60 dark:border-amber-800/40 rounded-xl p-3 text-xs text-amber-700 dark:text-amber-400 flex items-center gap-2">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    className={cn(
                      ...
                      primaryCategory === key
                        ? cn(
                            cat.bgColor,
                            cat.color,
                            "ring-2 ring-current/20 shadow-sm scale-[1.02]"
                          )
            ...
            <div className="space-y-2">
              ...
              <Textarea
                id="goal"
                placeholder="Sé específico: ¿qué vas a entregar, terminar, o avanzar?"
                value={goal}
            ...
            <div className="space-y-2">
              ...
              <Textarea
                id="obstacles"
                placeholder="Reuniones inesperadas, dependencias, falta de información..."
                value={obstacles}
            ...
            <div
              className={cn(
                ...
                committed
                  ? "bg-emerald-50/50 dark:bg-emerald-950/15 border-emerald-200/60 dark:border-emerald-800/40"
            ...
            <Button
              ...
              className="w-full h-11 rounded-xl bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white border-0 shadow-lg shadow-blue-600/25 hover:shadow-blue-600/40 transition-all duration-300 font-semibold text-base"
            >
              {submitting ? (
                "Guardando..."
              ) : (
            ...
            {!isValid && (
              <p className="text-[11px] text-center text-muted-foreground/60">
                {!committed
                  ? "Debes marcar el compromiso para continuar."
                  : goal.length < MIN_GOAL_LENGTH
                    ? `Tu meta necesita al menos ${MIN_GOAL_LENGTH} caracteres.`
  ...
  return (
    ...
    >
      ...
      <CardContent className="space-y-3">
        ...
        {/* Status messages */}
        {status === "warning" && (
          <div className="bg-amber-50/80 dark:bg-amber-950/15 border border-amber-200/60 dark:border-amber-800/40 rounded-xl p-2.5 text-xs text-amber-700 dark:text-amber-400 flex items-center gap-2">
        ...
        {status === "danger" && (
          <div className="bg-destructive/5 border border-destructive/20 rounded-xl p-2.5 text-xs text-red-700 dark:text-red-400 flex items-center gap-2">
        ...
        {status === "on-track" && actualHours >= intention.plannedHours && (
          <div className="bg-emerald-50/80 dark:bg-emerald-950/15 border border-emerald-200/60 dark:border-emerald-800/40 rounded-xl p-2.5 text-xs text-emerald-700 dark:text-emerald-400 flex items-center gap-2">
