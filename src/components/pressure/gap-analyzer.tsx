...
"text-sm font-bold tabular-nums",
 ...
 {/* Coverage bar */}
 <div className="bg-muted/40 px-3 py-2 space-y-1.5">
...
"text-sm font-bold tabular-nums",
 ...
 ?"text-green-600 dark:text-green-400": data.coveragePercent >= 50
...
"h-full rounded-full transition-all duration-700",
 data.coveragePercent >= 80
 ?"bg-green-500": data.coveragePercent >= 50
...
"es-MX",
 ...
 key={day.date}
 className={cn(
"flex flex-col items-center justify-center border p-2 transition-colors",
 day.gapCount === 0 &&
...
"text-red-700 dark:text-red-400",
 ...
 {data.habitPattern && (
 <div className="flex items-start gap-2 border border-amber-200 dark:border-amber-800/40 bg-amber-50/80 dark:bg-amber-950/15 px-3 py-2">
 ...
 <div className="border border-red-200 dark:border-red-800/40 bg-red-50/80 dark:bg-red-950/15 p-3 space-y-2">
...
function HourSquare({ slot }: { slot: HourSlot }) {
 ...
 <div
 className={cn(
"relative flex flex-col items-center justify-center aspect-square border-2 p-1 transition-all text-center min-h-[72px]",
...
"opacity-50",
 ...
 title={
 slot.entry
 ?`${formatHourShort(slot.hour)}: ${slot.entry.title}`:`${formatHourShort(slot.hour)}: ${slot.status ==="gap"|| slot.status ==="ghost"?"Sin registro": slot.status ==="current"?"Hora actual":"Futuro"}`}
 >
 {/* Hour label */}
 <span
 className={cn(
...
"text-[9px] leading-tight truncate max-w-full px-0.5 mt-0.5",
 ...
 ? slot.entry!.title.slice(0, 12) +"...": slot.entry!.title}
 </span>
 </>
 )}
 ...
 <Badge
 variant="destructive"className="absolute -top-1.5 -right-1.5 h-4 px-1 text-[8px]">
 !
 </Badge>
 </>
 )}
