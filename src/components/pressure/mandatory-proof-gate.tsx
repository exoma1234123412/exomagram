...
export function MandatoryProofGate({
  ...
  return (
    <div
      ...
      role="alertdialog"
      aria-modal="true"
      aria-label="Acceso bloqueado por registro incompleto"
    >
      ...
      <Card
        className={cn(
          ...
          "rounded-2xl border-red-500/30 bg-background p-0 shadow-2xl shadow-red-900/20"
        )}
      >
        {/* Header strip */}
        <div
          className="flex items-center gap-3 border-b border-red-500/20 bg-red-950/60 px-6 py-4 rounded-t-2xl"
          style={{ animation: "gate-pulse 3s ease-in-out infinite" }}
        >
          <div
            className="flex h-10 w-10 items-center justify-center rounded-xl bg-red-500/20"
            style={{ animation: "gate-lock-spin 4s ease-in-out infinite" }}
          >
        ...
        <div className="space-y-5 px-6 py-5">
          ...
          <div className="flex items-center justify-center gap-3 py-2">
            ...
            <p className="text-sm text-muted-foreground">
              Llevas{" "}
              <span
                className="font-mono font-bold text-red-500 tabular-nums"
                style={{
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between text-xs">
              ...
              >
                {canDismiss
                  ? "Completado"
                  : `Faltan ${MIN_REQUIRED_HOURS - totalHoursNow}`}
              </Badge>
            </div>
          ...
          {submittedSlots.length > 0 && (
            <div className="space-y-2">
              {submittedSlots.map((slot) => (
                <div
                  key={`done-${slot.hour}`}
                  className="flex items-center gap-3 rounded-xl bg-green-500/5 border border-green-500/20 px-4 py-3"
                >
                  ...
                  >
                            ...
                            <button
                              ...
                              className={cn(
                                ...
                                slot.category === key
                                  ...
                                  : "bg-accent/60 hover:bg-accent opacity-60 hover:opacity-100"
                              )}
                              title={cat.label}
                            >
                              <span>{cat.emoji}</span>
                    ...
                    <div className="flex gap-2">
                      <Input
                        placeholder={
                          slot.category
                            ? `¿Qué hiciste en ${CATEGORIES[slot.category as WorkCategory]?.label}?`
                      ...
                      <Button
                        type="button"
                        size="icon"
                        disabled={!isValid || slot.loading}
                        onClick={() => handleSlotSubmit(index)}
                        className={cn(
                          "rounded-xl border-0 shadow-lg transition-all",
                          isValid
