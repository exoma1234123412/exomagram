...
"flex items-center gap-1.5 text-xs font-mono font-semibold rounded-full px-3 py-1 border",
 countdown.expired
 ?"bg-red-500/15 text-red-600 dark:text-red-400 border-red-500/30": countdown.hours <= 2
 ?"bg-amber-500/15 text-amber-600 dark:text-amber-400 border-amber-500/30 animate-pulse":"bg-muted text-muted-foreground border-border",
 )}
 >
 <Clock className="size-3"/>
 {countdown.expired
 ?"Jornada finalizada":`Quedan ${countdown.hours}h ${String(countdown.minutes).padStart(2,"0")}m`}
 </div>
 </div>
 </CardHeader>
 ...
 <div
 className={cn(
"flex items-center justify-between px-3 py-2 text-sm font-medium border",
 position.isLast
 ?"bg-red-500/15 border-red-500/40 text-red-700 dark:text-red-300 animate-pulse": position.rank <= 3
 ?"bg-emerald-500/10 border-emerald-500/30 text-emerald-700 dark:text-emerald-300":"bg-muted border-border text-foreground",
 )}
 >
 ...
 key={item.id}
 className={cn(
"flex items-start gap-2.5 border px-3 py-2 transition-all duration-500",
 style.bg,
 style.border,
 style.text,
 item.variant ==="danger"&&"shadow-md shadow-red-500/20",
 item.variant ==="celebration"&&"shadow-emerald-500/20",
...
"size-3.5 mt-0.5 shrink-0 opacity-50",
 ...
 {/* CSS animations */}
 <style jsx>{`@keyframes slide-in-top {
 0% {
 opacity: 0;
