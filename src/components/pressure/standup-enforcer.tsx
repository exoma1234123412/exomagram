...
export function StandupEnforcer() {
 ...
 return (
 <Card
 size="sm"className="transition-all duration-300 hover:border-primary/30">
 <CardContent>
...
"flex items-center gap-3 p-3 border",
 isMyStandupLate
 ?"bg-yellow-50/50 dark:bg-yellow-950/10 border-yellow-200/60 dark:border-yellow-800/40":"bg-green-50/50 dark:bg-green-950/10 border-green-200/60 dark:border-green-800/40")}
 >
 <CheckCircle2
 className={cn(
"w-5 h-5 shrink-0",
...
"text-sm font-semibold",
 isMyStandupLate
 ?"text-yellow-700 dark:text-yellow-400":"text-green-700 dark:text-green-400")}
 >
 {isMyStandupLate
 ...
 {!myStandup && !showForm && (
 <Button
 variant="outline"className="w-full"onClick={() => setShowForm(true)}
 >
 ...
 <div>
 <button
 type="button"onClick={() => setShowBoard((prev) => !prev)}
...
"w-9 h-9 flex items-center justify-center shrink-0",
 level ==="pending"?"bg-blue-100 dark:bg-blue-900/40":"bg-red-100 dark:bg-red-900/40")}
 >
...
function StandupForm({
 ...
 <Textarea
 placeholder="Resumen de lo que lograste ayer..."value={formData.yesterday}
...
"border-red-300 dark:border-red-700 focus-visible:ring-red-500")}
 ...
 <Textarea
 placeholder="Plan específico para hoy..."value={formData.todayPlan}
...
"border-red-300 dark:border-red-700 focus-visible:ring-red-500")}
 ...
 <Input
 placeholder="¿Algo que te impide avanzar?"value={formData.blockers}
...
"w-full h-10 font-bold text-sm transition-all duration-300",
 isUrgent
 ?"bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 shadow-red-600/25 hover:shadow-red-600/40 text-white border-0":"bg-primary hover:from-blue-700 hover:to-blue-800 hover:shadow-blue-600/40 text-white border-0")}
 >
 {submitting ? (
 <>
 ...
 {escalation ==="pending"?"Enviar standup": escalation ==="overdue"?"Completar standup ahora":"Completar standup (tardío)"}
 </>
 )}
 </Button>
 ...
 {isUrgent && (
 <div className="bg-destructive/5 border border-destructive/20 p-2.5">
 <p className="text-[10px] text-red-600/80 dark:text-red-400/80 text-center font-medium leading-relaxed">
 {escalation ==="overdue"?`Tu equipo ya completó sus standups. Llevas ${formatOverdueTime(getMinutesPastDeadline())} de retraso.`:`Este standup se marcará como tardío y quedará en tu historial de Accountability.`}
 </p>
 </div>
 )}
 </form>
...
"border transition-all duration-200",
 hasStandup
 ?"bg-green-50/30 dark:bg-green-950/5 border-green-200/40 dark:border-green-800/20":"bg-red-50/30 dark:bg-red-950/5 border-red-200/40 dark:border-red-800/20",
 ...
 {/* Compact row */}
 <button
 type="button"onClick={() => hasStandup && setExpanded(!expanded)}
 disabled={!hasStandup}
 className={cn(
"w-full flex items-center gap-2.5 p-2.5 text-left",
 hasStandup &&"cursor-pointer hover:bg-green-50/50 dark:hover:bg-green-950/10")}
 >
 {/* Avatar */}
...
`· ${formatOverdueTime(member.overdueMinutes)} de retraso`}
 ...
 {hasStandup && isLate && (
 <Badge
 variant="outline"className="text-[9px] h-4 px-1 border-yellow-300 dark:border-yellow-700 text-yellow-600">
 Tardío
 </Badge>
 )}
 {hasStandup && (
