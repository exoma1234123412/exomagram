    ...
    >
      ...
      >
        ...
        {justChanged && previousTier && (
          <div
            className={cn(
              "mt-3 rounded-xl p-3 border flex items-center gap-2",
              changeDirection === "down"
                ? "bg-red-50/80 dark:bg-red-950/20 border-red-200 dark:border-red-800/50"
                : "bg-green-50/80 dark:bg-green-950/20 border-green-200 dark:border-green-800/50"
            )}
          >
            {changeDirection === "down" ? (
            ...
            >
              {changeDirection === "down"
                ? `Bajó de "${TIER_CONFIG[previousTier].label}" a "${config.label}"`
                : `Subió de "${TIER_CONFIG[previousTier].label}" a "${config.label}"`}
            </p>
          </div>
        )}
      </div>
      ...
      <CardContent className="pt-4 space-y-4">
        ...
        <div className="grid grid-cols-4 gap-2">
          <div className="bg-accent/40 rounded-xl p-2.5 text-center">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-medium text-muted-foreground">
                  Progreso hacia "{nextUpConfig.label}"
                </span>
              </div>
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-medium text-muted-foreground">
                  Margen antes de bajar a "{nextDownConfig.label}"
                </span>
              </div>
              <span
                className={cn(
            ...
            {TIER_ORDER.map((t) => {
              ...
              return (
                <div
                  key={t}
                  className={cn(
                    "flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-xs transition-all duration-300",
                    isCurrentTier
