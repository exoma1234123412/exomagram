...
function StoriesBar({
 ...
 <button
 onClick={() => scroll("left")}
 className="absolute left-0 top-1/2 -translate-y-1/2 z-10 w-7 h-7 rounded-full bg-background/90 border border-border/50 flex items-center justify-center hover:bg-accent transition-colors"aria-label="Desplazar izquierda">
...
"relative rounded-full p-[3px]",
 hasNoEntries
 ?"bg-gradient-to-br from-red-400 to-red-600":"bg-gradient-to-br from-primary/60 to-primary")}
 >
 <Avatar
 className={cn(
...
"absolute bottom-0.5 right-0.5 w-3 h-3 rounded-full border-2 border-background",
 ...
 className="absolute right-0 top-1/2 -translate-y-1/2 z-10 w-7 h-7 rounded-full bg-background/90 border border-border/50 flex items-center justify-center hover:bg-accent transition-colors"aria-label="Desplazar derecha">
...
function StoriesPopup({
 ...
 <div className="bg-card p-8 max-w-sm w-full text-center"onClick={(e) => e.stopPropagation()}>
 <Avatar className="w-16 h-16 mx-auto mb-4 ring-2 ring-background">
 <AvatarImage src={member.avatar_url ?? undefined} />
 ...
 <div
 className="bg-card max-w-md w-full overflow-hidden shadow-2xl"onClick={(e) => e.stopPropagation()}
 >
 {/* Progress bar */}
...
"h-full rounded-full bg-primary transition-all duration-300",
 ...
 <Button
 variant="ghost"size="sm"disabled={idx === latest.length - 1}
...
"postgres_changes",
 ...
 <Avatar className="w-6 h-6 mt-0.5 ring-2 ring-background">
 ...
 className="text-xs min-h-[32px] resize-none"onKeyDown={(e) => {
 ...
 className="h-8 w-8 shrink-0 bg-primary hover:from-blue-700 hover:to-blue-800 text-white border-0">
...
function FeedPost({
 ...
 {isLate && (
 <span
 className="inline-flex items-center gap-0.5 text-[10px] font-semibold text-orange-600 dark:text-orange-400"title={`Tardía (${entry.minutes_late}min)`}
 >
...
"text-[10px] font-semibold px-2 shrink-0",
 cat.color,
 cat.bgColor
 )}
 >
...
"inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-md",
 ...
 {entry.mood && (
 <span
 className="text-[10px] text-muted-foreground/80"title={`Ánimo: ${MOOD_LABELS[entry.mood]}`}
 >
 {"★".repeat(entry.mood)}
 {"☆".repeat(5 - entry.mood)}
 </span>
...
"postgres_changes",
 ...
 tab ==="siguiendo"? entries.filter((e) => followingIds.has(e.user_id))
 : entries;
 ...
 disabled={refreshing}
 className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground px-3 py-1.5 hover:bg-accent transition-colors">
 ...
 <div className="bg-accent/40 px-3 py-1.5">
...
"px-4 py-2.5 text-sm font-medium transition-colors relative",
 tab ==="para_ti"?"text-foreground":"text-muted-foreground hover:text-foreground")}
 >
 Para ti
 {tab ==="para_ti"&& (
 ...
 className={cn(
"px-4 py-2.5 text-sm font-medium transition-colors relative",
 tab ==="siguiendo"?"text-foreground":"text-muted-foreground hover:text-foreground")}
 >
 Siguiendo
 {followingIds.size > 0 && (
 ...
 <Button
 variant="outline"onClick={handleLoadMore}
