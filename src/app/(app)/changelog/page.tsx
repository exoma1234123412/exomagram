          ...
          {CHANGELOG.map((entry, idx) => {
            ...
            return (
              <div key={idx} className="relative flex gap-5">
                ...
                <div className="relative z-10 flex-shrink-0">
                  <div
                    className={cn(
                      ...
                      "bg-card border-2 border-border shadow-sm text-primary"
                    )}
                  >
                    {entry.icon}
                  </div>
                ...
                {/* Card */}
                <Card className="flex-1 transition-all duration-200 hover:shadow-lg hover:shadow-primary/5">
                  <CardContent className="p-4 sm:p-5">
                    {/* Top row: date + category */}
                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                      <Badge
                        variant="outline"
                        className="text-[10px] font-semibold rounded-lg px-2 py-0.5 tabular-nums tracking-tight"
                      >
                        {entry.date}
                      </Badge>
                      <Badge
                        className={cn(
                          "text-[10px] font-semibold rounded-lg px-2 py-0.5 border-0",
                          catStyle.bg,
                          catStyle.text
                        )}
                      >
