...
const byPerson = new Map<string, EntryWithProfile[]>();
 ...
 className="w-auto"max={new Date().toISOString().split("T")[0]}
 />
 ...
 size="icon"className="h-8 w-8"onClick={() => setPlaying(!playing)}
 ...
 size="sm"className="text-xs h-8"onClick={() => setSpeed(s)}
 >
 {["0.5x","1x","2x","4x"][i]}
 </Button>
 ))}
 ...
 <div
 className="h-full bg-gradient-to-r from-blue-500 to-blue-600 rounded-full transition-all duration-500"style={{ width:`${progress}%`}}
 />
 </div>
 {/* Hour markers */}
...
"w-1 h-1 rounded-full mx-auto",
 ...
 <Card
 key={userId}
 className="animate-in fade-in slide-in-from-bottom-2 duration-500 transition-all hover:border-primary/30">
...
"w-7 h-7 rounded-md flex items-center justify-center text-[10px] text-white font-medium animate-in zoom-in duration-300",
 ...
 {/* Latest entry */}
 <div className="mt-2 p-2 bg-accent/40">
