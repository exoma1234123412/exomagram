...
export default function BrainPage() {
 ...
 <div key={key}>
 <p className="text-[10px] font-semibold text-muted-foreground uppercase mb-1">{label}</p>
 <pre className="text-xs bg-accent/40 p-2 overflow-x-auto">{JSON.stringify(value, null, 2)}</pre>
 </div>
 );
 }
 ...
 <Button
 key={action.mode}
 variant="outline"size="sm"className="gap-1.5 text-xs"onClick={() => action.mode ==="ask"? null : sendMessage(action.mode)}
 disabled={loading}
 >
 ...
 <Button
 key={m.id}
 variant="ghost"size="sm"className="text-[10px] h-7 px-2"onClick={() => sendMessage("personality_profile", undefined, m.id)}
 disabled={loading}
 >
 ...
 <div className={cn(
"max-w-[85%] px-4 py-3",
 msg.role ==="user"?"bg-primary text-primary-foreground":"bg-accent/60 border")}>
 {msg.role ==="user"? (
 ...
 <div className="bg-accent/60 border px-4 py-3 flex items-center gap-2">
 ...
 placeholder="Pregúntale a Claude sobre el equipo..."disabled={loading}
 className="flex-1"/>
