    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 bg-grid-palantir min-h-screen">
      ...
      <div className="border border-border p-4 mb-8 corner-marks">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="label-mono text-muted-foreground/40">Filtros</span>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
          <div>
            <label className="label-mono text-muted-foreground/40 block mb-1">Desde</label>
            <Input
              type="date"
              value={startDate}
          ...
          <div>
            <label className="label-mono text-muted-foreground/40 block mb-1">Hasta</label>
            <Input
              type="date"
              value={endDate}
          ...
          <div>
            <label className="label-mono text-muted-foreground/40 block mb-1">Persona</label>
            <select
              value={filterUserId ?? ""}
          ...
          <div>
            <label className="label-mono text-muted-foreground/40 block mb-1">Categoría</label>
            <select
        ...
        <>
            ...
            <div className="border border-border p-4 corner-marks">
                ...
                {data.trends.map((t, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {t.trust_avg !== null && (
                        <div
                          className="absolute left-0 right-0 h-[2px] bg-primary/60 z-10"
                          style={{ bottom: `${trustHeight}%` }}
                        />
                      )}
                      {/* Hours bar */}
                      <div className="absolute bottom-0 left-0 right-0 flex flex-col justify-end" style={{ height: "100%" }}>
                        <div
                          className="bg-primary/40 transition-all duration-200 group-hover:bg-primary/70"
                          style={{ height: `${barHeight}%`, minHeight: t.hours > 0 ? 2 : 0 }}
                        />
                      </div>
                      {/* Tooltip on hover */}
              ...
              <div className="flex items-center gap-4 mt-3">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="font-mono text-[8px] text-muted-foreground/40 uppercase">Horas</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <div className="w-3 h-[2px] bg-primary/60" />
                  <span className="font-mono text-[8px] text-muted-foreground/40 uppercase">Trust Score</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mb-8">
            ...
            <div className="border border-border p-4 corner-marks overflow-x-auto">
              <div className="grid gap-[2px]" style={{ gridTemplateColumns: `40px repeat(7, 1fr)` }}>
                ...
                <div />
                {DOW_LABELS.map((d) => (
                  <div key={d} className="text-center font-mono text-[8px] text-muted-foreground/40 uppercase pb-1">
                    {d}
                  </div>
                ))}
                {/* Data rows */}
                ...
                return (
                  <div key={cat.category} className="flex items-center gap-3">
                    ...
                    <div className="w-20 shrink-0 text-right">
                      ...
                      <span className="font-mono text-[9px] text-muted-foreground/40 ml-1">{cat.percent}%</span>
                    </div>
                  </div>
                );
              })}
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <ScatterPlot
                title="Ánimo vs Horas"
                xLabel="Ánimo"
                yLabel="Horas"
                correlation={data.correlations.mood_vs_hours.correlation}
                data={data.correlations.mood_vs_hours.data}
              />
              <ScatterPlot
                title="Energía vs Evidencia"
                xLabel="Energía"
                yLabel="Evidencia %"
                correlation={data.correlations.energy_vs_proof.correlation}
                data={data.correlations.energy_vs_proof.data}
              />
              <ScatterPlot
                title="Horas vs Trust"
                xLabel="Horas"
                yLabel="Trust"
                correlation={data.correlations.hours_vs_trust.correlation}
                  ...
                  return (
                    <div key={f.type} className="flex items-center gap-3">
                      ...
                      <div className="w-16 shrink-0 text-right">
                        ...
                        <span className="font-mono text-[9px] text-muted-foreground/40 ml-1">{f.percent}%</span>
                      </div>
                    </div>
                  );
                })}
                  ...
                  return (
                    <div key={r.type} className="flex items-center gap-3">
                      ...
                      <div className="w-16 shrink-0 text-right">
                        ...
                        <span className="font-mono text-[9px] text-muted-foreground/40 ml-1">{r.percent}%</span>
                      </div>
                    </div>
                  );
                })}
  ...
  return (
    <div className="bg-accent/30 border border-border p-4">
      <div className="flex items-center gap-1.5 mb-2">
        <span className={cn("text-muted-foreground/50", accent === "destructive" && "text-destructive")}>
          {icon}
        </span>
    ...
    <>
      ...
      {days.map((d) => {
        ...
        return (
          <div
            key={`${hour}-${d}`}
            className="aspect-square border border-border/30 relative group"
            style={{ backgroundColor: count > 0 ? `oklch(0.70 0.14 200 / ${opacity})` : undefined }}
          >
            {count > 0 && (
  ...
  return (
    <div className="border border-border p-4 corner-marks">
      <div className="flex items-center justify-between mb-3">
        <span className="label-mono text-muted-foreground/40">{title}</span>
      ...
      <div className="relative bg-accent/10 border border-border/30" style={{ height: 140 }}>
        {data.length === 0 && (
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="font-mono text-[9px] text-muted-foreground/40">Sin datos</span>
          </div>
        )}
        {data.map(([x, y], i) => {
          ...
          return (
            <div
              key={i}
              className="absolute w-[5px] h-[5px] bg-primary/70 hover:bg-primary transition-colors"
              style={{
                left: `${left}%`,
                bottom: `${bottom}%`,
      ...
      <div className="flex justify-between mt-1">
        <span className="font-mono text-[7px] text-muted-foreground/30">{xLabel}</span>
        <span className="font-mono text-[7px] text-muted-foreground/30">{yLabel}</span>
      </div>
    </div>
  );
}
