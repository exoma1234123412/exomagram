...
function EntrySlide({ entry }: { entry: TimeEntry }) {
  ...
  return (
    ...
    >
      ...
      {entry.is_late && (
        <div
          className="absolute top-24 left-6 bg-red-600/80 backdrop-blur-sm border border-red-500/50 px-4 py-1.5 rounded-lg"
      ...
      {/* Large emoji */}
      <div className="text-7xl mb-6 drop-shadow-lg">{cat.emoji}</div>
        ...
        <div className="flex flex-wrap gap-2 justify-center mb-4 max-w-sm">
          {(entry.proof_urls as string[]).map((url, i) => (
            <a
              ...
              className="inline-flex items-center gap-1 text-xs bg-white/10 hover:bg-white/20 text-white/80 px-3 py-1.5 rounded-full transition-colors backdrop-blur-sm"
            >
              🔗 Evidencia {i + 1}
            </a>
          ))}
        ...
        <div className="flex justify-center gap-3">
          {Object.entries(REACTIONS).map(([key, r]) => (
            <button
              ...
              className="flex flex-col items-center gap-1 bg-white/5 hover:bg-white/15 border border-white/10 hover:border-white/30 rounded-2xl px-4 py-3 transition-all duration-200 active:scale-95"
              title={r.description}
            >
...
function StorySelector({
  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 z-30 pb-6 pt-3 bg-gradient-to-t from-black/80 via-black/40 to-transparent">
      <div
        ref={scrollRef}
        className="flex gap-3 overflow-x-auto px-6 scrollbar-hide justify-center"
            ...
            <div
              className={cn(
                ...
                i === currentIndex
                  ...
                  : story.seen
                    ? "bg-white/20"
                    : "bg-gradient-to-tr from-blue-400 via-cyan-400 to-green-400"
              )}
            >
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="absolute top-4 left-4 z-40 flex items-center gap-1.5 text-white/50 hover:text-white text-sm font-medium transition-colors bg-black/30 hover:bg-black/50 backdrop-blur-sm rounded-xl px-3 py-2"
      >
        <ArrowLeft className="w-4 h-4" />
        Volver
      </button>
