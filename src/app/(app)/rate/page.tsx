...
"w-5 h-5",
 ...
 ?"text-yellow-500 fill-yellow-500":"text-zinc-300 dark:text-zinc-600")}
 />
 ))}
 </div>
 ...
 <div
 key={entry.id}
 className="flex items-center gap-3 p-3 bg-accent/40">
...
"text-[10px] shrink-0",
 ...
 <Badge
 variant="outline"className="text-[9px] text-green-600 border-green-300 shrink-0">
 Con evidencia
 </Badge>
 )}
 </div>
 ...
 onClick={() => setRating(s)}
 className={cn(
"w-14 h-14 border-2 flex items-center justify-center transition-all duration-200",
 rating >= s
 ?"border-yellow-400 bg-yellow-50 dark:bg-yellow-950/30 scale-110":"border-border hover:border-yellow-300 hover:bg-accent/40")}
 >
 <Star
 className={cn(
"w-7 h-7 transition-colors",
 ...
 {rating === 1
 ?"Muy mal día": rating === 2
 ?"Día flojo": rating === 3
 ?"Día normal": rating === 4
 ?"Buen día":"Día excelente"}
 </p>
 )}
 ...
 placeholder="Ej: Buena consistencia pero faltó evidencia..."className="w-full px-3 py-2 border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/20"maxLength={140}
 />
 </div>
 ...
 className="w-full bg-primary text-white border-0 gap-2">
 {submitting ? (
