...
function AuditReportCard({
 ...
 ?"bg-green-500/5 border border-green-500/20":"bg-red-500/5 border border-red-500/20";
...
"gap-1 px-3 py-1 text-xs font-mono",
 ...
 <ScoreRow
 label="Horas registradas"detail={`${findings.hoursLogged}/8 horas`}
 points={Math.min(30, Math.round((findings.hoursLogged / 8) * 30))}
 max={30}
 />
 <ScoreRow
 label="Evidencia"detail={`${findings.proofPercent}% con prueba`}
 points={Math.round((findings.proofPercent / 100) * 30)}
 max={30}
 />
 <ScoreRow
 label="Cierre del día"detail={findings.hasCloseout ?"Enviado":"No enviado"}
 points={findings.hasCloseout ? 20 : 0}
 max={20}
 />
 <ScoreRow
 label="Puntualidad"detail={`${findings.lateEntries} tardías`}
...
"postgres_changes",
 ...
 disabled={!canSpin}
 size="lg"className="font-mono text-xs bg-primary text-primary-foreground hover:bg-primary/90 gap-2 px-8">
