...
"postgres_changes",
 ...
 (c) => c.liveStatus?.status ==="online").length;
 const idleCount = cards.filter(
 (c) => c.liveStatus?.status ==="idle").length;
 const deepWorkCount = cards.filter(
 (c) => c.liveStatus?.status ==="deep_work").length;
 const offlineCount = cards.filter(
 (c) => !c.liveStatus || c.liveStatus.status ==="offline").length;
 ...
 <div className="w-12 h-12 bg-gradient-to-br from-red-500 to-red-700 animate-pulse flex items-center justify-center">
 ...
 <Badge
 variant="outline"className="gap-1 border-green-300 dark:border-green-800 text-green-700 dark:text-green-400">
 <Wifi className="w-3 h-3"/>
 {onlineCount} online
 </Badge>
 {deepWorkCount > 0 && (
 <Badge
 variant="outline"className="gap-1 border-violet-300 dark:border-violet-800 text-violet-700 dark:text-violet-400">
 {deepWorkCount} deep work
 </Badge>
 )}
 <Badge
 variant="outline"className="gap-1 border-yellow-300 dark:border-yellow-800 text-yellow-700 dark:text-yellow-400">
 {idleCount} idle
 </Badge>
 <Badge
 variant="outline"className="gap-1 border-zinc-300 dark:border-zinc-700 text-zinc-500">
...
"font-bold",
 ...
 ?"text-green-600": transparencyPercent >= 50
 ...
 {/* ================================================================ */}
 {alerts.length > 0 && (
 <div className="mb-6 bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900 p-4">
...
"h-full rounded-full transition-all duration-500",
 ...
 ?"bg-gradient-to-r from-green-400 to-green-600": card.hoursLogged >= 4
 ?"bg-gradient-to-r from-yellow-400 to-yellow-600": card.hoursLogged > 0
 ?"bg-gradient-to-r from-red-400 to-red-600":"bg-zinc-600")}
 ...
 const isEmpty = !entry;
 const bgColor = entry
 ? HOUR_CATEGORY_COLORS[entry.category] ??"bg-blue-500": isPast
 ?"bg-zinc-300 dark:bg-zinc-700":"bg-muted/20 dark:bg-muted/10";
...
"w-full h-4 rounded-sm transition-all",
 ...
 title={
 entry
 ?`${hour}:00 - ${CATEGORIES[entry.category].emoji} ${CATEGORIES[entry.category].label}: ${entry.title}`:`${hour}:00 - ${isPast ?"Sin registro":"Pendiente"}`}
 />
 );
 })}
 </div>
 ...
 {/* Proof rate */}
 <div className="text-center p-1.5 bg-accent/30">
...
"text-sm font-bold tabular-nums",
 ...
 ?"text-green-600": card.proofPercent >= 50
 ?"text-yellow-600":"text-red-600")}
 >
 {card.proofPercent}%
 </span>
...
"text-sm font-bold tabular-nums",
 card.trustScore >= 80
 ?"text-green-600": card.trustScore >= 60
 ?"text-blue-600": card.trustScore >= 40
 ?"text-yellow-600":"text-red-600")}
 >
 {card.trustScore}
 </span>
 ...
 {/* ---- Row 5: Last entry ---- */}
 {card.lastEntry ? (
 <div className="bg-accent/20 px-2.5 py-2">
 ...
 <div className="bg-red-50 dark:bg-red-950/20 px-2.5 py-2 text-center">
