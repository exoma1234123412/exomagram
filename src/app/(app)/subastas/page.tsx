      ...
      <CardContent className="p-4">
        ...
        <div className="grid grid-cols-3 gap-2 mb-3">
          <div className="bg-accent/30 border border-border p-2 text-center">
            ...
            <p className="font-mono text-[9px] tracking-[0.18em] uppercase text-muted-foreground/40">Máximo</p>
          </div>
          <div
            className={cn(
          ...
          >
            ...
            <p className="font-mono text-[9px] tracking-[0.18em] uppercase text-muted-foreground/40">Mejor oferta</p>
          </div>
          <div className="bg-accent/30 border border-border p-2 text-center">
            ...
            <p className="font-mono text-[9px] tracking-[0.18em] uppercase text-muted-foreground/40">Ofertas</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
                ...
                <div className="flex items-center gap-2 flex-1">
                  <div className="relative flex-1">
                    <Input
                      ...
                      step="0.5"
                      min="0.5"
                      max={auction.max_hours}
                      placeholder={
                        lowestBid
                          ? `Menos de ${lowestBid.hours_bid}h`
                          : `Máx ${auction.max_hours}h`
                      }
                      value={bidValue}
                  ...
                  <Button
                    size="sm"
                    className="bg-primary font-mono text-xs gap-1"
                    disabled={
                      bidding ||
                      !bidValue ||
          ...
          {isClaimed && isCreator && (
            <>
              <Button
                size="sm"
                className="bg-green-600 hover:bg-green-700 text-white font-mono text-xs gap-1"
                disabled={acting}
              ...
              <Button
                size="sm"
                variant="outline"
                className="font-mono text-xs gap-1 text-red-600 border-red-300 dark:border-red-800 hover:bg-red-50 dark:hover:bg-red-950/20"
                disabled={acting}
                onClick={handleFail}
              >
                {acting ? (
      ...
      {totalAuctions > 0 && (
        <div className="grid grid-cols-4 gap-3 mb-8">
          <div className="bg-accent/30 border border-border p-3 text-center">
            ...
            <p className="font-mono text-[9px] tracking-[0.18em] uppercase text-muted-foreground/40">Abiertas</p>
          </div>
          <div className="bg-accent/30 border border-border p-3 text-center">
            <p className="text-2xl font-mono tabular-nums tracking-tight text-blue-600">
              {claimedAuctions.length}
            </p>
            <p className="font-mono text-[9px] tracking-[0.18em] uppercase text-muted-foreground/40">En progreso</p>
          </div>
          <div className="bg-accent/30 border border-border p-3 text-center">
            <p className="text-2xl font-mono tabular-nums tracking-tight">
              {totalBids}
            </p>
            <p className="font-mono text-[9px] tracking-[0.18em] uppercase text-muted-foreground/40">Ofertas totales</p>
          </div>
          <div className="bg-accent/30 border border-border p-3 text-center">
            <p className="text-2xl font-mono tabular-nums tracking-tight">
              {completionRate}%
            </p>
            <p className="font-mono text-[9px] tracking-[0.18em] uppercase text-muted-foreground/40">Cumplimiento</p>
          </div>
        </div>
      )}
      ...
      <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
        <DialogContent className="sm:max-w-md">
          ...
          <form onSubmit={createAuction} className="space-y-4">
            <div>
              <Label htmlFor="auction-title" className="font-mono text-[9px] tracking-[0.18em] uppercase text-muted-foreground/40">
                Tarea
              </Label>
              <Input
                id="auction-title"
                placeholder="Ej: Limpiar base de datos de prueba"
                value={title}
            ...
            <div>
              <Label
                ...
                className="font-mono text-[9px] tracking-[0.18em] uppercase text-muted-foreground/40"
              >
                Detalles (opcional)
              </Label>
              <Textarea
                id="auction-description"
                placeholder="Contexto adicional, requisitos, entregables..."
                value={description}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label
                  htmlFor="auction-max-hours"
                  className="font-mono text-[9px] tracking-[0.18em] uppercase text-muted-foreground/40"
                >
                  Máximo de horas
                </Label>
                <Input
                  id="auction-max-hours"
                  type="number"
                  min={1}
                  max={40}
                  step={1}
            ...
            {title.trim() && (
              <div className="bg-accent/30 border border-border p-3 text-sm">
                <p className="font-mono text-[9px] tracking-[0.18em] uppercase text-muted-foreground/40 mb-1">Vista previa</p>
                <p>
                  La tarea{" "}
