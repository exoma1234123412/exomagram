...
"postgres_changes",
 ...
 <Button variant="ghost"size="sm"onClick={goToPrevWeek} className="">
 ...
 <Button
 variant="ghost"size="sm"onClick={goToNextWeek}
 ...
 <span className="w-7 h-7 bg-primary/10 flex items-center justify-center text-xs font-bold text-primary shrink-0">
 {n}
 </span>
 <Input
 ...
 <textarea
 placeholder="Ej: Invito el almuerzo al equipo / Hago la presentacion que nadie quiere / Limpio el backlog"value={consequence}
 onChange={(e) => setConsequence(e.target.value)}
 rows={2}
 className="w-full border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 resize-none"/>
 </div>
...
function ActiveContract({
 ...
 <div
 className="h-full rounded-full bg-primary transition-all duration-500"style={{ width:`${total > 0 ? (delivered / total) * 100 : 0}%`}}
 />
 </div>
 ...
 <button
 key={i}
 type="button"onClick={() => isMine && onToggle?.(i)}
 disabled={!isMine}
 className={cn(
"w-full flex items-center gap-3 p-3 transition-all text-left",
 c.delivered
 ?"bg-green-50 dark:bg-green-950/20":"bg-accent/40 hover:bg-accent/60",
...
function GradedContract({
 ...
 return (
 <Card className={cn("transition-all duration-300 hover:border-primary/30", gs.border)}>
 <CardContent className="p-5">
 {/* Grade header */}
 <div className="flex items-start gap-4 mb-4">
 <div
 className={cn(
"w-16 h-16 flex flex-col items-center justify-center shrink-0",
 gs.bg
 )}
 >
...
"transition-all duration-300 hover:border-primary/30",
 ...
 <div className={cn("w-10 h-10 flex items-center justify-center", gs.bg)}>
...
function HistoryRow({ contract }: { contract: WeeklyContract }) {
 ...
 {/* Grade badge */}
 {contract.overall_grade ? (
 <div className={cn("w-10 h-10 flex items-center justify-center shrink-0", gs.bg)}>
 <span className={cn("text-lg font-black tabular-nums", gs.color)}>
 {contract.overall_grade}
 </span>
 </div>
 ) : (
 <div className="w-10 h-10 flex items-center justify-center shrink-0 bg-accent/40">
 <Clock className="w-4 h-4 text-muted-foreground"/>
 </div>
 )}
