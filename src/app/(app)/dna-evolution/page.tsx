...
function MutationsList({ months }: { months: MonthData[] }) {
 ...
 {mutations.map((m, i) => {
 const isPositive =
 m.type ==="proof"? m.delta > 0
 : m.category ==="deep_work"||
 m.category ==="planning"||
 m.category ==="learning"? m.delta > 0
 : m.category ==="meeting"|| m.category ==="blocked"? m.delta < 0
 : true;
...
"tabular-nums border font-bold",
 isPositive
 ?"bg-green-500/10 text-green-600 border-green-500/30":"bg-red-500/10 text-red-600 border-red-500/30")}
 >
 {m.delta > 0 ?"+":""}
 {m.delta}%
 </Badge>
...
"font-bold",
 ...
 ?"text-green-600": month.avgDailyHours >= 5
...
"font-bold",
 month.proofRate >= 80
 ?"text-green-600": month.proofRate >= 50
 ?"text-yellow-600":"text-red-600")}
 >
 {month.proofRate}%
 </span>
...
"flex items-center gap-3 p-3 border transition-all duration-300",
 isSelected
 ?"border-primary/30 bg-primary/5":"border-border/50 hover:border-border")}
 >
 {/* Avatar */}
...
export default function DnaEvolutionPage() {
 ...
 className={cn(
"flex items-center gap-2 px-3 py-2 border text-sm transition-all duration-200",
 isSelected
 ?"border-primary bg-primary/5 text-foreground font-medium":"border-border/50 text-muted-foreground hover:border-border hover:bg-accent/40")}
 >
 <Avatar className="ring-2 ring-background"size="sm">
 <AvatarImage
...
"border",
 ...
 ?"Los patrones de trabajo se han mantenido consistentes.": selectedDna.driftScore <= 60
 ?"Se han detectado cambios moderados en los patrones de trabajo.":"Los patrones de trabajo han cambiado significativamente entre meses."}
 </p>
 </div>
...
"h-full rounded-full transition-all duration-700",
 selectedDna.driftScore <= 30
 ?"bg-green-500": selectedDna.driftScore <= 60
