...
function CountdownTimer({ deadline }: { deadline: string }) {
 ...
 <div
 className={cn(
"flex items-center gap-2 px-4 py-3 font-mono text-2xl font-bold tabular-nums tracking-tight transition-all duration-300",
 isExpired
 ?"bg-red-500/10 text-red-600 dark:text-red-400": isUrgent
 ?"bg-red-500/10 text-red-600 dark:text-red-400 animate-pulse":"bg-accent/40 text-foreground")}
 >
 <Timer
 className={cn(
"w-5 h-5",
 isExpired
 ?"text-red-500": isUrgent
 ?"text-red-500 animate-pulse":"text-primary")}
 />
 {isExpired ? (
...
"postgres_changes",
 ...
 ?"a": activePairing?.user_b === userId
 ?"b": null;
 const myVerified =
 mySide ==="a"? activePairing?.user_a_verified
 : mySide ==="b"? activePairing?.user_b_verified
 : false;
 ...
 onClick={handleSpin}
 disabled={!canSpin}
 size="lg"className="bg-primary hover:from-blue-700 hover:to-blue-800 text-white border-0 gap-2 text-base px-8">
 ...
 {/* Slot A */}
 <div className="flex-1 bg-accent/40 p-6 border-2 border-primary/20">
 <p className="text-xs text-muted-foreground text-center mb-2 uppercase tracking-wider">
 Jugador A
 </p>
 <SpinSlot
 names={memberNames}
 finalName=""spinning={true}
 duration={2500}
 />
 </div>
 ...
 <div
 className={cn(
"flex-1 p-4 border transition-all duration-300",
 activePairing.user_a_verified
 ?"border-green-500/30 bg-green-50 dark:bg-green-900/10":"border-border bg-card")}
 >
...
"?"}
 ...
 <div className="bg-accent/40 p-3 space-y-1">
 <div className="flex items-center gap-2">
 <Badge
 className={cn(
"text-[10px]",
 ...
 disabled={verifying}
 className="w-full mt-3 bg-primary hover:from-blue-700 hover:to-blue-800 text-white border-0 gap-2">
...
"text-[10px]",
 ...
 {/* User A */}
 <div className="flex items-center gap-2">
 <Avatar className="w-7 h-7 ring-2 ring-background"size="sm">
 <AvatarImage
