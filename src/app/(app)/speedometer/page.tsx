...
"postgres_changes",
 ...
 ?"#3b82f6": velocityScaleRatio >= 0.5
 ?"#22c55e": velocityScaleRatio >= 0.25
 ?"#f59e0b":"#ef4444";
 ...
 className="w-full max-w-[420px] h-auto"preserveAspectRatio="xMidYMid meet">
 <defs>
 ...
 fill="none"stroke={currentZoneColor}
 strokeWidth={outerR - innerR}
 strokeLinecap="butt"opacity={0.6}
 style={{
 ...
 x2={outerTick.x}
 y2={outerTick.y}
 stroke="currentColor"strokeWidth={isMajor ? 2 : 0.5}
 className="text-muted-foreground"/>
 {isMajor && (
 <text
 x={labelPos.x}
 y={labelPos.y}
 textAnchor="middle"dominantBaseline="central"className="fill-muted-foreground/60"fontSize="10"fontFamily="monospace">
 {val}
 </text>
 )}
 </g>
 ...
 x={cx}
 y={cy + 45}
 textAnchor="middle"dominantBaseline="central"fontSize="42"fontFamily="monospace"fontWeight="bold"fill={currentZoneColor}
 >
 {currentVelocity}
 </text>
 <text
 x={cx}
 y={cy + 65}
 textAnchor="middle"dominantBaseline="central"fontSize="9"fontFamily="monospace"className="fill-muted-foreground/50"letterSpacing="0.15em">
 ENTRADAS / HORA
 </text>
 ...
 x2={markerOuter.x}
 y2={markerOuter.y}
 stroke="currentColor"strokeWidth="2"strokeDasharray="4,2"className="text-muted-foreground"/>
 </g>
 );
 })()}
 </svg>
 ...
 <p className="font-mono text-[9px] tracking-[0.18em] uppercase text-muted-foreground">Actual</p>
 <p className="font-mono tabular-nums tracking-tight text-2xl font-bold"style={{ color: currentZoneColor }}>
 {currentVelocity}
 </p>
 </div>
 <div className="bg-accent/30 border border-border px-4 py-2 text-center">
 <p className="font-mono text-[9px] tracking-[0.18em] uppercase text-muted-foreground">Esperado</p>
 <p className="font-mono tabular-nums tracking-tight text-2xl font-bold text-muted-foreground">
 {expectedVelocity}
 </p>
 </div>
 <div className="bg-accent/30 border border-border px-4 py-2 text-center">
 <p className="font-mono text-[9px] tracking-[0.18em] uppercase text-muted-foreground">Ratio</p>
 <p className={cn(
...
"text-sm font-mono truncate",
 ...
 <span className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground border border-border px-2 py-0.5">
 Inactivo
 </span>
 )}
 ...
 <p className="font-mono text-[8px] text-muted-foreground uppercase">1h</p>
 </div>
 ...
 <p className="font-mono text-[8px] text-muted-foreground uppercase">Hoy</p>
 </div>
 </div>
 );
 })}
 ...
 <div
 className="absolute left-0 right-0 border-t border-dashed border-muted-foreground/30 z-10 pointer-events-none"style={{
 bottom:`${Math.min((expectedVelocity / Math.max(maxBucketCount, expectedVelocity)) * 100, 100) * 0.8 + 10}%`,
 }}
 >
 <span className="absolute -top-3.5 right-0 font-mono text-[8px] text-muted-foreground uppercase">
 ...
 const barColor =
 bucket.count === 0
 ?"bg-red-500/30": bucket.count >= expectedVelocity
 ?"bg-blue-500/60": bucket.count >= expectedVelocity * 0.5
 ?"bg-green-500/50":"bg-amber-500/50";

 return (
 <div key={bucket.hour} className="flex-1 flex flex-col items-center justify-end h-full gap-1">
 {/* Count label */}
 <span className="font-mono tabular-nums text-[9px] text-muted-foreground">
