...
export default function HallOfShamePage() {
 ...
 (f) => f.user_id === uid && f.flag_type ==="suspicious_pattern").length;
...
"border transition-colors p-3",
 m.totalFlags >= 10
 ?"border-2 border-red-500/50 bg-red-500/5": m.totalFlags >= 5
 ?"border-red-500/30 bg-red-500/3":"border-border")}
 >
...
"font-mono text-[10px] font-bold tracking-[0.1em] px-1.5 py-0.5 border",
 m.totalFlags >= 10
 ?"text-red-500 border-red-500/40 bg-red-500/10": m.totalFlags >= 5
 ?"text-amber-500 border-amber-500/30 bg-amber-500/10":"text-muted-foreground border-border bg-accent/20")}
 >
 {m.totalFlags} FLAGS
 </span>
 </div>
...
"flex items-start gap-3 p-2.5 border-b border-border/50 font-mono text-xs",
 ...
 />
 <span className="font-mono text-[10px] tabular-nums text-muted-foreground w-20">
...
"font-mono text-[11px] font-bold shrink-0 w-28 truncate",
 event.severity ==="high"?"text-red-500": event.severity ==="medium"?"text-amber-500":"text-foreground")}
 >
 {event.name}
 </span>
 ...
 ?`${Math.abs(Math.round(brutalStats.bestImprovement.delta))} menos flags`:"Sin mejora notable"}
 </p>
 )}
 </div>
 ...
 ?`+${Math.round(brutalStats.worstDeterioration.delta)} más flags`:"Sin deterioro notable"}
 </p>
 )}
 </div>
 </div>
