...
function RankHistoryChart({
 ...
 className="w-[120px] h-[48px]"preserveAspectRatio="none">
 <path
 d={pathD}
 fill="none"stroke="currentColor"strokeWidth="2"className="text-primary"strokeLinecap="round"strokeLinejoin="round"/>
 {points.map((p, i) => (
 <circle
 key={i}
 cx={p.x}
...
export default function PowerRankingsPage() {
 ...
 <Button
 variant="outline"size="icon"className=""onClick={() => setWeekStart(subWeeks(weekStart, 1))}
 >
 <ChevronLeft className="w-4 h-4"/>
 </Button>
 <Button
 variant="outline"size="sm"className=""onClick={() =>
...
"bg-primary text-white border-0")}
 ...
 {playerOfTheWeek && (
 <Card className="border-yellow-300/60 dark:border-yellow-700/40 shadow-yellow-500/10 ring-1 ring-yellow-300/30 overflow-hidden">
 ...
 <Avatar className="w-20 h-20 ring-4 ring-yellow-400/40">
 <AvatarImage
 ...
 {/* === Caída Libre === */}
 {freeFall && (
 <Card className="border-red-300/60 dark:border-red-800/40 shadow-red-500/10">
...
"border-red-200 dark:border-red-900/40")}
 ...
 {/* Avatar */}
 <Avatar className="w-12 h-12 ring-2 ring-background mt-0.5">
 <AvatarImage
 src={r.profile.avatar_url ?? undefined}
 />
 <AvatarFallback>
...
"tabular-nums",
 r.proofPercent >= 80
 ?"text-green-600": r.proofPercent >= 50
 ...
 {/* Hot take */}
 <p className="text-xs italic text-muted-foreground mt-2 bg-accent/40 px-3 py-1.5">
 {r.rank === 1 && (
...
"text-3xl font-bold tabular-nums tracking-tight",
 r.powerScore >= 80
 ?"text-green-600": r.powerScore >= 60
 ?"text-blue-600": r.powerScore >= 40
 ?"text-yellow-600":"text-red-600")}
 >
 {r.powerScore}
 </p>
 ...
 <div
 key={r.profile.id}
 className="flex items-center gap-3 p-2 bg-accent/30">
