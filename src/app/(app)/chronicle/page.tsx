...
function generateNarrative(
 ...
 ?"excepcional": week.gradeScore >= 70
 ?"aceptable": week.gradeScore >= 50
 ?"mediocre": week.gradeScore >= 30
 ?"preocupante":"desastrosa";
...
`Categoría dominante: ${catInfo.label} (${topCategory[1]} horas). ${
 topCategory[0] ==="meeting"?"Demasiado tiempo en reuniones.": topCategory[0] ==="break"?"Demasiados descansos.": topCategory[0] ==="deep_work"?"Bien enfocado en trabajo profundo.":""}`);
 }
 }
 ...
 ?`${firstName} demostró ser confiable esta semana. El equipo puede contar con este rendimiento.`: week.gradeScore >= 70
 ?`${firstName} cumplió con lo mínimo aceptable. Hay margen de mejora considerable.`: week.gradeScore >= 50
 ?`${firstName} está por debajo del estándar. Si esto continúa, habrá consecuencias.`: week.gradeScore >= 30
 ?`${firstName} necesita una intervención urgente. Este nivel de rendimiento perjudica al equipo.`:`${firstName} fue un lastre para el equipo esta semana. Sin mejora inmediata, esto es insostenible.`;
...
export default function ChroniclePage() {
 ...
 disabled={loading}
 className="gap-2 font-mono text-xs"size="sm">
 ...
 <Button
 variant="ghost"size="icon"onClick={() => setWeekOffset((o) => o - 1)}
 disabled={loading}
 className="w-7 h-7">
...
"text-2xl font-mono font-bold tabular-nums tracking-tight",
 ...
 ?"text-green-600": weekStats.proofRate >= 50
...
"text-2xl font-mono font-bold tabular-nums tracking-tight",
 ...
 ?"text-green-600": weekStats.lateRate <= 30
 ?"text-yellow-600":"text-red-600")}
 >
 ...
 <div key={i}>
 {section.title && (
 <p className="font-mono text-[9px] tracking-[0.18em] uppercase text-muted-foreground mb-2 border-b border-border/50 pb-1">
 {section.title}
 </p>
 )}
 <div
...
"border-l-2 border-red-500 pl-4 text-red-700 dark:text-red-400")}
 ...
 const intensity =
 day.totalHours === 0
 ?"bg-red-500/20 border-red-500/30": day.totalHours < 4
 ?"bg-yellow-500/20 border-yellow-500/30": day.totalHours < 7
 ?"bg-blue-500/20 border-blue-500/30":"bg-green-500/20 border-green-500/30";
...
"p-2 border text-center transition-colors duration-200",
 ...
 const emoji =
 type ==="verified"?"OK": type ==="suspicious"?"??": type ==="impressive"?"--":"--";
 const label =
 type ==="verified"?"Confirmo": type ==="suspicious"?"Sospechoso": type ==="impressive"?"Impresionante":"Me ayudó";
 return (
 <div
 key={type}
