...
type NotificationType =
 |"reaction_received"|"flag_raised"|"streak_milestone"|"entry_logged"|"closeout_reminder"|"standup_reminder"|"title_change"|"buddy_alert"|"goal_completed";
...
"group relative flex items-start gap-3.5 px-4 py-3.5 transition-all duration-200 cursor-pointer",
 !notification.read
 ?"bg-primary/[0.03] hover:bg-primary/[0.06]":"hover:bg-accent/50",
 ...
 <div
 className={cn(
"flex-shrink-0 w-9 h-9 flex items-center justify-center mt-0.5",
 config.bgColor
 )}
 >
...
"text-sm leading-snug",
 !notification.read
 ?"font-semibold text-foreground":"text-foreground/90")}
 >
...
function EmptyState({ filtered }: { filtered: boolean }) {
 ...
 {filtered
 ?"No hay notificaciones de este tipo. Prueba con otro filtro.":"Estás al día — no hay notificaciones pendientes."}
 </p>
 </div>
 );
}
...
"postgres_changes",
 ...
 <style>{`@keyframes notif-slide-in {
 from {
 opacity: 0;
...
`}</style>
 ...
 : tab.value !=="all"? filterNotifications(notifications, tab.value).length
 : null;
...
"flex items-center gap-1.5 px-3 py-2.5 text-xs sm:text-sm font-medium transition-colors relative whitespace-nowrap shrink-0",
 ...
 <Badge
 variant="secondary"className="ml-1 text-[10px] px-1.5 py-0 h-4 rounded-full">
 {unreadCount}
 </Badge>
 )}
 {tab.value !=="all"&&
 ...
 <Link
 href="/settings"className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
