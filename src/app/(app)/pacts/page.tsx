...
export default function PactsPage() {
 ...
 return (
 <Card key={pact.id} className={cn("transition-all duration-300 hover:border-primary/30", !isActive &&"opacity-60")}>
 ...
 <div
 className="h-full bg-gradient-to-r from-blue-500 to-blue-600 rounded-full transition-all duration-500"style={{ width:`${Math.min((pact.creator_progress / pact.target_value) * 100, 100)}%`}}
 />
 </div>
 </div>
 <div>
 ...
 <div className="h-2 bg-muted rounded-full overflow-hidden">
 <div
 className="h-full bg-blue-500 rounded-full transition-all duration-500"style={{ width:`${Math.min((pact.partner_progress / pact.target_value) * 100, 100)}%`}}
 />
 </div>
 </div>
 </div>
 ...
 size="sm"onClick={() => setDurationDays(d.toString())}
 >
 {d}d
 </Button>
 ))}
 ...
 <Button onClick={handleCreate} disabled={creating || !partnerId || !title.trim()} className="w-full bg-primary hover:from-blue-700 hover:to-blue-800 text-white border-0 hover:shadow-blue-600/40 transition-all duration-300 font-semibold">
