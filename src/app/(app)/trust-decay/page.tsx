    ...
    <div className="relative flex flex-col items-center justify-end h-36 w-12">
      ...
      {!isExtinguished ? (
        <div className="relative flex items-end justify-center mb-[-2px] z-10">
          ...
          <div
            className="candle-flame-inner absolute bottom-0 w-2.5 rounded-full bg-gradient-to-t from-white/80 to-white/20"
            style={{
    ...
    <Card className="transition-all duration-300 hover:shadow-lg hover:shadow-primary/5 overflow-hidden">
      <CardContent className="p-4 flex flex-col items-center gap-3">
        ...
        <Badge
          variant="secondary"
          className={cn(
            "tabular-nums tracking-tight text-xs font-bold px-2.5 py-0.5 rounded-lg",
          ...
          <div className="flex items-center justify-between text-muted-foreground">
            ...
            <span className="font-medium text-foreground tabular-nums">
              {member.hoursSinceLastEntry < 1
                ? "< 1h"
                : `${Math.round(member.hoursSinceLastEntry)}h`}
            </span>
          </div>
          <div className="flex items-center justify-between text-muted-foreground">
            <span className="flex items-center gap-1">
          ...
          <div className="flex items-center justify-between text-muted-foreground">
            ...
            <span className="font-medium text-foreground tabular-nums">
              {member.hoursSinceLastEntry > 0
                ? `-${Math.min(Math.round(member.hoursSinceLastEntry), member.baseScore)}/h`
                : "0/h"}
            </span>
          </div>
        </div>
        ...
        <span
          className={cn(
            ...
            member.decayedScore >= 80
              ...
              : member.decayedScore >= 60
                ? "text-yellow-700 dark:text-yellow-400"
                : member.decayedScore >= 40
                  ? "text-orange-700 dark:text-orange-400"
                  : member.decayedScore >= 20
...
function TeamAverageCandle({ members }: { members: MemberCandle[] }) {
  ...
  return (
    <Card className="transition-all duration-300 hover:shadow-lg hover:shadow-primary/5 mb-8">
      <CardContent className="p-6">
        <div className="flex flex-col sm:flex-row items-center gap-6">
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <Badge
              variant="secondary"
              className={cn(
                "tabular-nums tracking-tight text-sm font-bold px-3 py-1 rounded-lg",
        ...
        <div className="mt-4 flex items-center justify-center gap-2">
          ...
          >
            {avgScore >= 80
              ? "El equipo esta ardiendo - excelente ritmo"
              : avgScore >= 60
                ? "Llama estable - buen ritmo de trabajo"
                : avgScore >= 40
                  ? "La llama se debilita - alguien necesita registrar"
                  : avgScore >= 20
                    ? "Zona cr\u00edtica - varias velas a punto de apagarse"
                    : "Emergencia - el equipo esta a oscuras"}
          </span>
        </div>
      </CardContent>
    </Card>
