        ...
        <div
          ...
          title={
            slot.logged && slot.hasProof
              ? `${formatHourShort(slot.hour)}: Registrada con prueba`
              : slot.logged
                ? `${formatHourShort(slot.hour)}: Sin prueba (deuda continua)`
                : `${formatHourShort(slot.hour)}: No registrada`
          }
        >
          ...
          {members.map((m) => {
            ...
            return (
              <div key={m.userId} className={cn(borderClass, paddingClass)}>
                ...
                {m.slots.length > 0 && (
                  <div className="mt-3">
                    <p className="font-mono text-[9px] tracking-[0.12em] uppercase text-muted-foreground/40 mb-1">
                      Horas de hoy
                    </p>
