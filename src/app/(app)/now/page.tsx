...
"absolute -bottom-0.5 -right-0.5 w-4 h-4 rounded-full border-2 border-card",
 ...
 {inFlow ? (
 <Badge
 variant="outline"className="text-[10px] shrink-0 text-primary border-primary/30 bg-primary/5 dark:bg-primary/10 animate-pulse">
 <Sparkles className="w-3 h-3 mr-0.5"/>
 En flow - No molestar
 </Badge>
 ) : (
 <Badge
 variant="outline"className={cn("text-[10px] shrink-0", config.color)}
 >
 {config.label}
 </Badge>
 )}
 ...
 {s.status !=="offline"?`Activo desde ${timeAgo(s.started_at)}`:`Ultimo heartbeat ${timeAgo(s.last_heartbeat)}`}
 </p>
 </div>
 </div>
 </CardContent>
