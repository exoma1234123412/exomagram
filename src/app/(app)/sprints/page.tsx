...
export default function SprintsPage() {
 ...
 <div className="w-10 h-10 bg-gradient-to-br from-indigo-500 to-indigo-700 animate-pulse"/>
 ...
 <Button
 size="sm"variant="outline"className="gap-1 text-green-600 border-green-200 hover:bg-green-50"onClick={() => endSprint(activeSprint.id,"completed")}
 >
 <CheckCircle2 className="w-3 h-3"/> Completar
 </Button>
 <Button
 size="sm"variant="outline"className="gap-1 text-red-500 border-red-200 hover:bg-red-50"onClick={() => endSprint(activeSprint.id,"cancelled")}
 >
...
"h-full rounded-full transition-all duration-700",
 ...
 ?"bg-gradient-to-r from-red-400 to-red-500": sprintProgress.percent >= 60
 ?"bg-gradient-to-r from-amber-400 to-amber-500":"bg-gradient-to-r from-primary to-primary/80")}
 ...
 <div
 className="h-full bg-primary rounded-full transition-all"style={{
...
"h-full rounded-full transition-all duration-700",
 ...
 {/* SVG lines */}
 <svg
 className="absolute inset-0 w-full"style={{ height:"calc(100% - 24px)"}}
 ...
 .join("")}
 stroke="currentColor"className="text-muted-foreground"strokeWidth="2"strokeDasharray="6 4"vectorEffect="non-scaling-stroke"/>
 {/* Actual line */}
 <polyline
 points={burndownData.points
 .filter((p) => p.isPast)
 ...
 .join("")}
 stroke="currentColor"className={cn(
...
"absolute left-1/2 -translate-x-1/2 w-2 h-2 rounded-full",
 ...
 <CardContent className="p-4">
 <button
 className="w-full text-left"onClick={() => {
 if (expanded) {
 ...
 variant="outline"className={cn(
"text-[10px]",
 sprint.status ==="completed"?"border-green-300 text-green-600":"border-red-300 text-red-500")}
 >
 {sprint.status ==="completed"?"Completado":"Cancelado"}
 </Badge>
 {stats && (
 <Badge
 className={cn(
...
"text-[10px] shrink-0",
 ...
 <Input
 placeholder="Ej: Sprint 12 - Lanzamiento MVP"value={formName}
 ...
 <Textarea
 placeholder="Que queremos lograr en este sprint..."value={formDesc}
 ...
 <Input
 type="number"min="1"placeholder="40"value={formGoalHours}
 ...
 <Input
 type="number"min="1"max="100"placeholder="80"value={formFocusPercent}
 ...
 <Button
 type="submit"className="w-full gap-2"disabled={submitting || !formName || !formStart || !formEnd}
 >
 {submitting ? (
"Creando...") : (
 <>
