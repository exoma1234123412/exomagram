...
export default function SurveillancePage() {
  ...
  const stats = useMemo(() => {
    ...
    const activeSessions = members.filter(
      (m) => m.liveStatus && m.liveStatus.status !== "offline"
    ).length;
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-0.5 flex-wrap">
            <span className={cn("text-[9px] font-mono", isOnline ? statusConfig.color : "text-muted-foreground/50")}>
    ...
    <div className="border-t border-border px-3 sm:px-4 py-4 space-y-4 bg-accent/10">
      ...
      <div>
        ...
        {weekDayStats.length > 0 ? (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border">
                  <th className="text-left font-mono text-[9px] text-muted-foreground/40 uppercase tracking-wider pb-1 pr-3">Dia</th>
                  <th className="text-right font-mono text-[9px] text-muted-foreground/40 uppercase tracking-wider pb-1 px-2">Horas</th>
                  <th className="text-right font-mono text-[9px] text-muted-foreground/40 uppercase tracking-wider pb-1 px-2">Evid%</th>
                  <th className="text-right font-mono text-[9px] text-muted-foreground/40 uppercase tracking-wider pb-1 px-2">Tardes</th>
                  <th className="text-right font-mono text-[9px] text-muted-foreground/40 uppercase tracking-wider pb-1 px-2">Calif</th>
                </tr>
              </thead>
              <tbody>
                {weekDayStats.map((d) => (
          ...
          <p className="font-mono text-[10px] text-muted-foreground/50">Sin datos esta semana</p>
        )}
      </div>
...
function MiniStat({
  ...
  return (
    <div className="bg-accent/20 border border-border p-2">
      <p className="font-mono text-[8px] tracking-[0.18em] uppercase text-muted-foreground/40 mb-0.5 truncate">
        {label}
      </p>
        ...
        <div className="min-w-[360px]">
          ...
          <div className="flex items-center gap-0">
            ...
            {hours.map((h) => (
              <div
                key={h}
                className="flex-1 text-center font-mono text-[7px] text-muted-foreground/30"
              >
                {h}
              </div>
            ))}
          ...
          {days.map((day, di) => (
            <div key={day} className="flex items-center gap-0">
              <div className="w-8 shrink-0 font-mono text-[8px] text-muted-foreground/40 text-right pr-1">
                {dayLabels[di]}
              </div>
              {hours.map((h) => {
                ...
                return (
                  <div
                    key={`${day}-${h}`}
                    className="flex-1 aspect-square m-[1px] border border-border/20"
                    style={{
                      backgroundColor:
                        count > 0
                          ? `oklch(0.65 0.12 258 / ${intensity})`
                          : "transparent",
                    }}
      ...
      {events.slice(0, 50).map((event) => {
        ...
        return (
          <div key={event.id} className="flex items-start gap-3 p-2 hover:bg-accent/10 transition-colors">
            ...
            <span className="font-mono text-[10px] text-muted-foreground/50 tabular-nums shrink-0 w-14">
