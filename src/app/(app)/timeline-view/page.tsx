...
function TimelineBlock({
  ...
  if (entry) {
    ...
    return (
      <Tooltip>
        <TooltipTrigger
          className={cn(
            "h-10 rounded-lg transition-all duration-200 cursor-default relative group",
            "bg-gradient-to-br",
            gradient,
        ...
        <TooltipContent side="top" className="max-w-[260px] rounded-xl">
  ...
  if (isMissing) {
    return (
      <Tooltip>
        <TooltipTrigger
          className={cn(
            "h-10 rounded-lg transition-all duration-200 cursor-default relative",
        ...
        >
          {isGapStart && isInConsecutiveGap && (
            <div className="absolute -top-5 left-0 right-0 flex justify-center z-10 pointer-events-none">
              <span className="text-[9px] font-bold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/80 px-1.5 py-0.5 rounded-md border border-red-200 dark:border-red-800 whitespace-nowrap shadow-sm">
  ...
  // Future hour - empty
  return (
    <div className="h-10 rounded-lg bg-muted/10 dark:bg-muted/5" />
  );
}
...
function MemberTimelineRow({
  ...
  return (
    <div
      className={cn(
        "flex items-stretch gap-3 py-3 px-4 rounded-2xl transition-all duration-200",
        "hover:bg-accent/30",
        hasAlarmingGaps &&
    ...
    >
      ...
      <div className="flex flex-col items-center gap-1.5 w-[80px] shrink-0 justify-center">
        ...
        {row.liveStatus && row.liveStatus.status !== "offline" ? (
          ...
          <span className="flex items-center gap-1 text-[9px] text-muted-foreground/50 font-medium">
      ...
      <div className="flex flex-col justify-center gap-1 w-[110px] shrink-0 pl-2 border-l border-border/30">
        ...
        <div className="flex items-center gap-1">
          <AlertTriangle
            className={cn(
              "w-3 h-3",
              row.gaps > 0 ? "text-red-500" : "text-muted-foreground/30"
            )}
          />
          <span
            className={cn(
              "text-xs font-semibold tabular-nums",
              row.gaps > 0 ? "text-red-600" : "text-muted-foreground/50"
            )}
          >
...
function CurrentTimeMarker({ dateStr }: { dateStr: string }) {
  ...
  return (
    ...
    >
      {/* Vertical line */}
      <div className="absolute top-0 bottom-0 w-0.5 bg-red-500 shadow-lg shadow-red-500/30" />
      {/* Pulse dot at top */}
      ...
      <div className="absolute -top-6 -translate-x-1/2 left-[1px]">
        <span className="text-[9px] font-bold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/80 px-1.5 py-0.5 rounded-md border border-red-200 dark:border-red-800 whitespace-nowrap shadow-sm tabular-nums">
          {format(now, "HH:mm")}
        </span>
      </div>
    </div>
    ...
    <TooltipProvider>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-6 mt-4">
          {/* Date navigation */}
          <div className="flex items-center gap-2 bg-card/80 border border-border/50 rounded-2xl p-2">
            <Button
              variant="ghost"
              size="icon"
