...
function MemberRow({ member: m }: { member: MemberState }) {
 ...
 ?"bg-red-500": m.isGhost
 ?"bg-amber-500": m.liveStatus && m.liveStatus.status !=="offline"? LIVE_STATUS_CONFIG[m.liveStatus.status]?.dotColor ??"bg-green-500": m.entriesToday > 0
 ?"bg-green-500":"bg-slate-400";
...
"absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-background",
 ...
 <Badge
 variant="secondary"className="font-mono text-[9px] bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 border border-amber-300 dark:border-amber-800">
 ...
 <Badge
 variant="secondary"className="font-mono text-[9px] tabular-nums bg-accent/30 border border-border text-muted-foreground">
 {m.entriesToday}e
 </Badge>
 )}
 </div>
...
function TrustRow({
 ...
 const trendColor =
 m.delta7d > 0
 ?"text-green-600 dark:text-green-400": m.delta7d < 0
 ...
 const barColor =
 score >= 80
 ?"bg-green-500": score >= 50
 ?"bg-amber-500": score > 0
 ?"bg-red-500":"bg-muted-foreground/10";
 return (
...
"text-2xl font-mono font-bold tabular-nums tracking-tight",
 m.currentScore >= 80
 ?"text-green-600 dark:text-green-400": m.currentScore >= 50
...
"text-2xl font-mono font-bold tabular-nums tracking-tight",
 ...
 ?"text-green-600 dark:text-green-400": teamMood !== null && teamMood >= 3
 ?"text-amber-600 dark:text-amber-400": teamMood !== null
...
"text-2xl font-mono font-bold tabular-nums tracking-tight",
 ...
 ?"text-green-600 dark:text-green-400": teamEnergy !== null && teamEnergy >= 3
 ?"text-amber-600 dark:text-amber-400": teamEnergy !== null
 ?"text-red-600 dark:text-red-400":"")}>
...
function PulseRow({ member: m }: { member: MemberPulse }) {
 ...
 ?"text-green-600 dark:text-green-400": m.avgMood7d !== null && m.avgMood7d >= 3
 ?"text-amber-600 dark:text-amber-400": m.avgMood7d !== null
 ?"text-red-600 dark:text-red-400":"text-muted-foreground";

 const energyColor =
 m.avgEnergy7d !== null && m.avgEnergy7d >= 4
 ?"text-green-600 dark:text-green-400": m.avgEnergy7d !== null && m.avgEnergy7d >= 3
 ?"text-amber-600 dark:text-amber-400": m.avgEnergy7d !== null
 ?"text-red-600 dark:text-red-400":"text-muted-foreground";
 ...
 ?`${m.moodEntries + m.energyEntries} datos en 7d`:"Sin datos de pulso"}
 </p>
 </div>
 ...
 {/* Energy */}
 <div className="flex flex-col items-center shrink-0 w-14">
 <Zap className="w-3.5 h-3.5 text-muted-foreground mb-0.5"/>
