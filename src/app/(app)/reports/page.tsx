...
type MetricKey =
 |"hours"|"proof_rate"|"trust_score"|"mood"|"energy"|"flags"|"streaks"|"closeouts"|"reactions"|"shoutouts"|"categories"|"late_rate";
...
`/api/reports/builder?report_id=${reportId}&org_id=${orgId}`,
 ...
 return sort.direction ==="asc"? String(aVal).localeCompare(String(bVal))
 ...
 setSort((prev) => {
 if (prev?.key === key) {
 return prev.direction ==="asc"? { key, direction:"desc"}
 : null;
 }
 ...
 <label className="font-mono text-[9px] text-muted-foreground mb-1 block">
 Desde
 </label>
 <input
...
"flex items-center gap-2 px-3 py-2 border transition-colors text-left cursor-pointer",
 checked
 ?"border-primary/40 bg-primary/8 text-foreground":"border-border bg-accent/20 text-muted-foreground hover:border-primary/20")}
 >
 <div
 className={cn(
...
"px-3 py-2 border font-mono text-[10px] transition-colors cursor-pointer",
 groupBy === g.key
 ?"border-primary/40 bg-primary/8 text-foreground":"border-border text-muted-foreground hover:border-primary/20")}
 >
 {g.label}
 </button>
 ))}
...
"flex items-center gap-2 px-2.5 py-1.5 border transition-colors text-left cursor-pointer",
 checked
 ?"border-primary/40 bg-primary/8":"border-border/50 hover:border-primary/20")}
 >
 <div
 className={cn(
...
"w-3 h-3 border flex items-center justify-center shrink-0",
 ...
 placeholder="Nombre del reporte"className="w-full bg-accent/30 border border-border px-3 py-2 font-mono text-xs text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary/50"/>
 </div>
 </div>

 {/* Formato */}
 <div>
 <span className="font-mono text-[9px] tracking-[0.18em] uppercase text-muted-foreground mb-1 block">
 Formato
 </span>
...
"px-4 py-2 border font-mono text-[10px] transition-colors cursor-pointer",
 exportFormat ==="json"?"border-primary/40 bg-primary/8 text-foreground":"border-border text-muted-foreground hover:border-primary/20")}
 >
 JSON
 </button>
 <button
...
"px-4 py-2 border border-l-0 font-mono text-[10px] transition-colors cursor-pointer",
 exportFormat ==="csv"?"border-primary/40 bg-primary/8 text-foreground":"border-border text-muted-foreground hover:border-primary/20")}
 >
 CSV
 </button>
 </div>
...
"hover:bg-primary/90")}
 ...
 {col.key ==="categories"? formatCategoryMap(row[col.key])
 : col.key ==="group_label"? String(row[col.key] ??"-")
 : row[col.key] != null
 ? String(row[col.key])
 :"-"}
 </td>
 ...
 {ci === 0
 ?"TOTAL": col.key ==="categories"?"-": summary[`total_${col.key}`] != null
