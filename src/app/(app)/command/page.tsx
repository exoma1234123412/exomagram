...
export default function CommandCenterPage() {
 ...
 <div className="palantir-divider text-muted-foreground mb-4">Inteligencia por persona</div>
 ...
 {flags.length > 0 && <>
 <div className="palantir-divider text-muted-foreground mb-4">Alertas activas</div>
 ...
 <span className="data-cell text-muted-foreground">{f.date}</span>
 </div>
 ); })}
 {flags.length > 8 && <p className="text-[10px] font-mono text-muted-foreground text-center pt-1">+{flags.length - 8} mas</p>}
 </div>
 </>}
 </>}
 ...
 {sel.latestTrust !== undefined && <div className="text-right"><p className="label-mono text-muted-foreground">Trust Score</p><p className={cn("data-number text-2xl", trustColor(sel.latestTrust))}>{sel.latestTrust}</p></div>}
 {sel.isGhost && <span className="system-badge text-red-600 border-red-300">GHOST</span>}
 </div>
 </div>
 </div>
 <div className="palantir-divider text-muted-foreground mb-3">Trust Score (14 dias)</div>
 <div className="card-palantir p-4 mb-6">
 {userTrust14.length === 0 ? <p className="text-center text-muted-foreground font-mono text-xs py-6">Sin datos</p> : (
 ...
 <span className="data-cell text-[8px] text-muted-foreground">{t.score}</span>
 ...
 <span className="data-cell text-[7px] text-muted-foreground">{t.date.slice(5)}</span>
 </div>
 ); })}
 </div>
 )}
 </div>
 <div className="palantir-divider text-muted-foreground mb-3">Horas por categoria (7 dias)</div>
 <div className="card-palantir p-4 mb-6">
 {Object.keys(catH).length === 0 ? <p className="text-center text-muted-foreground font-mono text-xs py-6">Sin entradas</p> : (
 ...
 <div className="palantir-divider text-muted-foreground mb-3">Flags activas</div>
 ...
 <span className="data-cell text-muted-foreground shrink-0">{f.date}</span>
 </div>
 ); })}
 </div>
 </>}
 ...
 <div className="palantir-divider text-muted-foreground mb-3">Ultimo insight AI</div>
 <div className="card-palantir p-4 corner-marks">
 {grade && <div className="flex items-center gap-2 mb-2"><span className="label-mono text-muted-foreground">Grade</span><span className={cn("data-number text-lg", grade <="B"?"text-green-600": grade ==="C"?"text-yellow-600":"text-red-600")}>{grade}</span></div>}
 ...
 {riskLoading && <div className="flex flex-col items-center py-16 gap-3"><Brain className="w-10 h-10 text-primary animate-pulse"/><p className="font-mono text-xs text-muted-foreground">Analizando patrones...</p><p className="font-mono text-[10px] text-muted-foreground">15-30 segundos</p></div>}
 ...
 <div className="card-palantir p-4 corner-marks mb-6">
 <p className="label-mono text-muted-foreground mb-2">Resumen del equipo</p>
 ...
 {p.signals?.length > 0 && <div className="mb-3"><p className="label-mono text-muted-foreground mb-1">Senales</p><div className="flex flex-wrap gap-1.5">{p.signals.map((s, j) => <span key={j} className="system-badge text-muted-foreground border-border">{s}</span>)}</div></div>}
 ...
 {!riskData && !riskLoading && !riskError && <div className="flex flex-col items-center py-16 gap-3"><div className="w-16 h-16 border border-border flex items-center justify-center"><Crosshair className="w-7 h-7 text-muted-foreground"/></div><p className="font-mono text-xs text-muted-foreground">Ejecuta el analisis para ver predicciones</p></div>}
 </>}

 {tab ==="auditoria"&& <>
 <div className="grid grid-cols-3 gap-3 mb-8">
 <div className="bg-accent/30 border border-border p-3"><p className="label-mono text-muted-foreground">Reviews AI</p><p className="data-number text-xl mt-1">{reviews.length}</p></div>
 <div className="bg-accent/30 border border-border p-3"><p className="label-mono text-muted-foreground">Impacto Trust Prom.</p><p className={cn("data-number text-xl mt-1", (reviews.length > 0 ? Math.round(reviews.reduce((s, r) => s + r.trust_impact, 0) / reviews.length) : 0) >= 0 ?"text-green-600":"text-red-600")}>{reviews.length > 0 ? (Math.round(reviews.reduce((s, r) => s + r.trust_impact, 0) / reviews.length) > 0 ?"+":"") + Math.round(reviews.reduce((s, r) => s + r.trust_impact, 0) / reviews.length) : 0}</p></div>
 <div className="bg-accent/30 border border-border p-3"><p className="label-mono text-muted-foreground">Flags activas</p><p className="data-number text-xl mt-1">{flags.length}</p></div>
 </div>
 ...
 <div className="palantir-divider text-muted-foreground mb-3">Distribucion de flags</div>
 ...
 <div className="palantir-divider text-muted-foreground mb-3">Reviews AI recientes</div>
 {reviews.length === 0 ? <div className="flex flex-col items-center py-12 gap-3"><div className="w-16 h-16 border border-border flex items-center justify-center"><Eye className="w-7 h-7 text-muted-foreground"/></div><p className="font-mono text-xs text-muted-foreground">Sin reviews recientes</p></div> : (
 ...
 <span className="data-cell text-muted-foreground ml-auto">{r.date}</span>
 </div>
...
function Stat({ label, value, icon: Icon, accent, danger }: { label: string; value: number; icon: typeof Users; accent?: boolean; danger?: boolean }) {
 ...
 <Icon className={cn("w-3.5 h-3.5", danger ?"text-red-500": accent ?"text-green-500":"text-muted-foreground")} />
...
function MiniTrend({ label, data }: { label: string; data: Record<string, number[]> }) {
 ...
 return (
 <div>
 <div className="palantir-divider text-muted-foreground mb-3">{label}</div>
 <div className="card-palantir p-4">
 {days.length === 0 ? <p className="text-center text-muted-foreground font-mono text-xs py-4">Sin datos</p> : (
 <div className="flex items-end gap-1.5 h-16">
 {avgs.map((v, i) => (
 <div key={i} className="flex-1 flex flex-col items-center gap-0.5">
 <span className="data-cell text-[7px] text-muted-foreground">{v}</span>
 <div className={cn("w-full", v >= 4 ?"bg-green-500/60": v >= 3 ?"bg-yellow-500/60":"bg-red-500/60")} style={{ height:`${Math.max(8, (v / 5) * 100)}%`}} />
 <span className="data-cell text-[6px] text-muted-foreground">{days[i]?.slice(8)}</span>
 </div>
 ))}
 </div>
 )}
