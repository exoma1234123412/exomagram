...
"w-2 h-2 rounded-full",
 silenceLevel ==="critical"?"bg-red-500 animate-pulse": silenceLevel ==="warning"?"bg-amber-500 animate-pulse":"bg-green-500")}
 />
 <span className="font-mono text-[9px] tracking-[0.18em] uppercase text-muted-foreground">
 {silenceLevel ==="critical"?"Equipo detenido": silenceLevel ==="warning"?"Silencio prolongado":"En vivo"}
 </span>
 </div>
 </div>
...
"border mb-8 p-4 transition-colors duration-500",
 silenceLevel ==="critical"?"border-red-500/50 bg-red-500/5": silenceLevel ==="warning"?"border-amber-500/30 bg-amber-500/5":"border-border bg-accent/20")}
 >
 ...
 className={cn(
"w-4 h-4",
 silenceLevel ==="critical"?"text-red-500 animate-pulse": silenceLevel ==="warning"?"text-amber-500":"text-green-500")}
 />
 <div>
...
"font-mono text-2xl font-bold tabular-nums tracking-tight",
 silenceLevel ==="critical"?"text-red-500": silenceLevel ==="warning"?"text-amber-500":"text-foreground")}
 >
...
"flex-1 transition-all duration-300",
 count > 0
 ?"bg-primary":"bg-muted-foreground/10")}
 style={{
 ...
 <span className="font-mono text-[9px] tabular-nums text-muted-foreground ml-auto">
 {entries.length} hoy
 </span>
 </div>
...
"border-b border-border/50 px-3 py-2.5 transition-all duration-500",
 ...
 {/* Timestamp */}
 <span className="font-mono text-[10px] tabular-nums text-muted-foreground shrink-0 mt-0.5 w-16">
...
"w-2 h-2 rounded-full shrink-0 mt-1.5",
 ...
 <StatRow
 label="Total entradas"value={String(stats.total)}
 icon={<Activity className="w-3 h-3"/>}
 />
 <StatRow
 label="Tasa/hora"value={stats.rate.toFixed(1)}
 icon={<Clock className="w-3 h-3"/>}
 />
 <StatRow
 label="Última entrada"value={
 stats.lastEntry
 ...
 <StatRow
 label="Mayor silencio"value={`${formatDurationShort(stats.longestSilence.durationMinutes)} (${formatTimestamp(stats.longestSilence.startTime)}–${formatTimestamp(stats.longestSilence.endTime)})`}
