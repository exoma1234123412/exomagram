...
const ACTION_LABELS: Record<string, { label: string; emoji: string; color: string }> = {
 ...
 entry_updated: { label:"Entrada editada", emoji:"--", color:"text-yellow-600"},
 entry_deleted: { label:"Entrada borrada", emoji:"--", color:"text-red-600"},
 ...
 flag_resolved: { label:"Flag resuelta", emoji:"OK", color:"text-green-600"},
...
export default function AuditPage() {
 ...
 size="sm"onClick={() => { setFilterAction(""); setPage(0); }}
 >
 Todos
 </Button>
 ...
 size="sm"onClick={() => { setFilterAction(a); setPage(0); }}
 >
 ...
 return (
 <div key={entry.id} className="flex items-center gap-3 py-2.5 px-3 hover:bg-muted/50 transition-colors">
 ...
 <Button
 variant="outline"size="sm"disabled={entries.length < pageSize}
 onClick={() => setPage((p) => p + 1)}
 >
